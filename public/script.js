...
function getDefaultData(){
  return {
    ...
    categoriasProdutos:["Vaporizadores","Máquinas","Acessórios","Outros"],
    categoriasFluxo:[
...
function onCompraProductChange(nome){
  var p = (appData.pFornecedores||[]).find(function(x){return x.produto === nome;});
  if(p){
    var valInput = document.getElementById('cpValor');
    var fornInput = document.getElementById('cpForn');
    if(valInput) valInput.value = p.valor || 0;
    if(fornInput && p.fornecedor) fornInput.value = p.fornecedor;
  }
}
function openCompraModal(compra){
  var isEdit=!!compra;
  var sortedForn = (appData.fornecedores || []).slice().sort(function(a, b) {
    return (a.nome || '').localeCompare(b.nome || '');
  });
  var fornOpts=sortedForn.map(function(f){return'<option value="'+f.nome+'"'+(compra&&compra.fornecedor===f.nome?' selected':'')+'>'+f.nome+'</option>';}).join('');
  ...
  var pFornList = (appData.pFornecedores || []).map(function(p){ return p.produto; });
  var uniqueP = Array.from(new Set(pFornList)).sort(function(a,b){ return (a||'').localeCompare(b||''); });
  var prodOpts = uniqueP.map(function(p){ return '<option value="'+p+'">'; }).join('');

  document.getElementById('cadastroModalTitle').textContent=isEdit?'Editar Compra':'Nova Compra';
  document.getElementById('cadastroModalBody').innerHTML='<div class="form-row"><div class="form-group"><label>Data</label><input type="date" class="form-control" id="cpData" value="'+(compra?compra.data:todayStr())+'"></div><div class="form-group"><label>Vencimento</label><input type="date" class="form-control" id="cpVenc" value="'+(compra?compra.vencimento||'':'')+'"></div></div>' +
    '<div class="form-group"><label>Produto *</label><input class="form-control" id="cpProd" list="cpProdList" value="'+(compra?compra.produto:'')+'" oninput="onCompraProductChange(this.value)" placeholder="Digite para buscar..."><datalist id="cpProdList">'+prodOpts+'</datalist></div>' +
    ...
    '<div class="form-group"><label>Situação</label><select class="form-control" id="cpSit">'+sitOpts+'</select></div><div class="form-group"><label>Obs</label><textarea class="form-control" id="cpObs" rows="2">'+(compra?compra.obs||'':'')+'</textarea></div>';
...
function renderComprasResultPanel(list){var panel=document.getElementById('comprasResultPanel');if(!panel)return;var total=list.reduce(function(s,c){return s+((c.quantidade||1)*(c.valorUnit||0));},0);var pago=list.filter(function(c){return c.situacao==='Pago';}).reduce(function(s,c){return s+((c.quantidade||1)*(c.valorUnit||0));},0);var devendo=list.filter(function(c){return c.situacao==='Devendo';}).reduce(function(s,c){return s+((c.quantidade||1)*(c.valorUnit||0));},0);panel.innerHTML='<div class="card"><div class="card-header"><span>Total Compras Anual</span></div><div class="card-value">'+formatCurrency(total)+'</div><div class="card-sub">'+list.length+' compra(s)</div></div><div class="card"><div class="card-header"><span>Pago</span></div><div class="card-value text-success">'+formatCurrency(pago)+'</div></div><div class="card"><div class="card-header"><span>Devendo</span></div><div class="card-value text-danger">'+formatCurrency(devendo)+'</div></div>';}
...
function renderVendasTable(vendas){var tbody=document.getElementById('vendasBody');if(!tbody)return;if(vendas.length===0){tbody.innerHTML='<tr><td colspan="10" style="text-align:center;padding:40px;color:var(--text-muted)">Nenhuma venda encontrada</td></tr>';return;}var sitOpts=(appData.situacaoVenda||[]);var entregaOpts=(appData.situacaoEntrega||[]);tbody.innerHTML=vendas.map(function(v){var total=(v.quantidade||1)*(v.valorUnit||0);var liq = v.valorLiquido !== undefined ? v.valorLiquido : total;var totalDisplay = formatCurrency(liq); if(liq < total){ totalDisplay = '<span title="Bruto: '+formatCurrency(total)+'">'+formatCurrency(liq)+'</span> <small style="display:block; font-size:9px; color:var(--text-muted)">Taxa: '+v.taxa+'%</small>'; } var sitColor='';if(v.situacao==='Pago'){sitColor='color:#22c55e;font-weight:bold;';}else if(v.situacao==='Devendo'){sitColor='color:#ef4444;font-weight:bold;';}var sitSelect='<select class="form-control" style="min-width:100px;padding:4px 6px;font-size:12px;font-weight:bold;'+sitColor+'" onchange="changeVendaField('+v.id+',\'situacao\',this.value)">'+sitOpts.map(function(s){var optColor='';if(s==='Pago'){optColor='color:#22c55e;font-weight:bold;';}else if(s==='Devendo'){optColor='color:#ef4444;font-weight:bold;';}return'<option value="'+s+'" style="'+optColor+'"'+(v.situacao===s?' selected':'')+'>'+s+'</option>';}).join('')+'</select>';var entColor='';if(v.entrega==='Entregue OK'){entColor='color:#22c55e;font-weight:bold;';}else if(v.entrega==='Não Entregue'){entColor='color:#eab308;font-weight:bold;';}var entregaSelect='<select class="form-control" style="min-width:100px;padding:4px 6px;font-size:12px;font-weight:bold;'+entColor+'" onchange="changeVendaField('+v.id+',\'entrega\',this.value)">'+entregaOpts.map(function(s){var optColor='';if(s==='Entregue OK'){optColor='color:#22c55e;font-weight:bold;';}else if(s==='Não Entregue'){optColor='color:#eab308;font-weight:bold;';}return'<option value="'+s+'" style="'+optColor+'"'+(v.entrega===s?' selected':'')+'>'+s+'</option>';}).join('')+'</select>';var acoes='<button class="btn btn-sm btn-outline" onclick="viewVenda('+v.id+')">👁️</button> <button class="btn btn-sm btn-primary" onclick="editVenda('+v.id+')">✏️</button> <button class="btn btn-sm btn-danger" onclick="deleteVenda('+v.id+')">🗑️</button>';return'<tr><td>'+formatDate(v.data)+'</td><td>'+(v.produto||'-')+'</td><td>'+(v.cliente||'-')+'</td><td>'+(v.quantidade||1)+'</td><td>'+formatCurrency(v.valorUnit)+'</td><td>'+totalDisplay+'</td><td>'+(v.formaPagamento||'-')+'</td><td>'+sitSelect+'</td><td>'+entregaSelect+'</td><td>'+acoes+'</td></tr>';}).join('');}
...
function onVendaProductChange(nome){
  ...
  if(p){
    var tipoVenda = document.getElementById('vnTipoVenda') ? document.getElementById('vnTipoVenda').value : 'Direta';
    var valInput = document.getElementById('vnValor');
    if(valInput) {
      if(tipoVenda === 'Revenda') {
        valInput.value = p.valorRevenda || 0;
      } else {
        valInput.value = p.valorVenda || 0;
      }
    }
    updateVendaCalc();
  }
...
function updateVendaCalc(){
  ...
  var sit = document.getElementById('vnSit') ? document.getElementById('vnSit').value : '';
  var parcialArea = document.getElementById('vnParcialArea');
  
  if(parcialArea) parcialArea.style.display = (sit === 'Parcial') ? 'block' : 'none';
  ...
  if(calcArea){
    var valPago = (sit === 'Parcial') ? parseFloat(document.getElementById('vnValorPago').value || 0) : (sit === 'Pago' ? liqVal : 0);
    var saldo = liqVal - valPago;
    ...
    calcArea.innerHTML = '<div style="background:var(--bg-tertiary); padding:10px; border-radius:8px; margin-top:10px; border:1px solid var(--border-color); margin-bottom:15px">' +
      ...
      (sit === 'Parcial' ? '<div style="display:flex; justify-content:space-between; border-top:1px dashed var(--border-color); margin-top:8px; padding-top:8px"><span>Valor Já Pago:</span><strong class="text-info">'+formatCurrency(valPago)+'</strong></div>' +
      '<div style="display:flex; justify-content:space-between; font-size:1.1em"><span>Saldo Devedor:</span><strong class="text-danger">'+formatCurrency(saldo)+'</strong></div>' : '') +
    '</div>';
  }
}
...
var devendo = liquido - pago;
panel.innerHTML='<div class="card"><div class="card-header"><span>Total Vendas Anual</span></div><div class="card-value">'+formatCurrency(total)+'</div><div class="card-sub">'+list.length+' venda(s)</div></div><div class="card card-accent"><div class="card-header"><span>Total Líquido (Real)</span></div><div class="card-value text-success">'+formatCurrency(liquido)+'</div></div><div class="card"><div class="card-header"><span>Total Recebido</span></div><div class="card-value text-success">'+formatCurrency(pago)+'</div></div><div class="card"><div class="card-header"><span>Total à Receber</span></div><div class="card-value text-danger">'+formatCurrency(devendo)+'</div></div>';}
...
function renderGarantiasTable(list){var tbody=document.getElementById('garantiasBody');if(!tbody)return;if(list.length===0){tbody.innerHTML='<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--text-muted)">Nenhuma garantia</td></tr>';return;}var sortedList = list.slice().sort(function(a,b){return (a.vencimento||'').localeCompare(b.vencimento||'');});tbody.innerHTML=sortedList.map(function(g){var dias=calcDiasGarantia(g.vencimento);var sitAuto=getGarantiaSituacaoAuto(g.vencimento,g.situacao);return'<tr><td>'+(g.produto||'-')+'</td><td>'+(g.cliente||'-')+'</td><td>'+formatDate(g.inicio)+'</td><td>'+formatDate(g.vencimento)+'</td><td>'+formatDiasGarantia(dias,sitAuto)+'</td><td>'+situacaoBadge(sitAuto)+'</td><td><button class="btn btn-sm btn-primary" onclick="editGarantia('+g.id+')">✏️</button> <button class="btn btn-sm btn-danger" onclick="deleteGarantia('+g.id+')">🗑️</button></td></tr>';}).join('');}
...
function renderConfiguracoesPage(){
  ...
  var catProdHtml=buildDragSection('🏷️','Categorias de Produto','categoriasProdutos','Nova categoria...');
